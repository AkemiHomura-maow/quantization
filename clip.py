...
def distiller_quantize(x, num_bits, alpha):
    min_q_val, max_q_val = 0, 2 ** num_bits
    scale = torch.tensor(2 ** num_bits / alpha)
    ## 1. Round to integer domain, 2. Clamp to 0 - 2**n, 3. Dequantize by dividing by scale
    return torch.clamp(torch.round(scale * x), min_q_val, max_q_val).div_(scale).numpy()
...
def mse_histogram_clip(bin_x, bin_y, num_bits, alpha):
   ...
   idx = bin_x > alpha
   mse = np.sum((bin_x[idx] - alpha)**2 * bin_y[idx])
...
def find_clip_mmse(values, num_bits):
    # Build histogram
    max_val = torch.max(values).cpu().numpy()
    bin_num = 10000
    bin_y, bin_edges = torch.histogram(values, bins=bin_num, density=True)
    bin_y, bin_edges = bin_y.cpu().numpy(), bin_edges.cpu().numpy()
    bin_x = 0.5*(bin_edges[:-1] + bin_edges[1:])
    bin_y /= np.sum(bin_y)

    alphas = np.arange(1/bin_num, 1 + 1/bin_num, 1/bin_num) * max_val
    mses = [ mse_histogram_clip(bin_x, bin_y, num_bits, alpha)
             for alpha in alphas ]

    alpha_best = alphas[np.argmin(mses)]
    print(" MSE alpha_best = %5.3f / %5.3f" % (alpha_best, max_val))
    return alpha_best
...
def find_clip_entropy(values, num_bits):
    num_quantized_bins = 2**num_bits

    max_val, opt_th = _get_optimal_threshold_one_sided(
        values, num_quantized_bins=num_quantized_bins)

    alpha_best = opt_th
    print(" Entropy alpha_best = %5.3f / %5.3f" % (alpha_best, max_val))
    return alpha_best
...
def _get_optimal_threshold(arr, num_bins=1000, num_quantized_bins=255):
...
def _get_optimal_threshold_one_sided(arr, num_bins=100000, num_quantized_bins=256):
    """Given a dataset, find the optimal threshold for quantizing it.
    Ref: http://on-demand.gputechconf.com/gtc/2017/presentation/s7310-8-bit-inference-with-tensorrt.pdf
    """
    
    max_val = torch.max(arr).cpu().numpy()
    hist, hist_edges = torch.histogram(arr, bins=num_bins, range=(0, max_val))
    hist, hist_edges = hist.cpu().numpy(), hist_edges.cpu().numpy()
    # assert np.allclose(hist_edges[1],
    #                    0, rtol=1e-1, atol=1e-3)

    thresholds = np.zeros(len(range(2*num_quantized_bins - 1, num_bins, 2*num_quantized_bins)))
    divergence = np.zeros_like(thresholds)
    quantized_bins = np.zeros(num_quantized_bins, dtype=np.int32)
    # After scaling:
    # 0 to 0.5 = 0.5 bin, 
    # 0.5 to 2**b - 0.5 = 1 bin every int, 
    # 2**b - 0.5 to 2**b = 0.5 bin.
    # In total = 2**b bins
    # Binning (merging) can only be done when (i + 1) // num_quantized_bins is a multiple of 2 
    # i: number of currently incoporated data bins
    for indexi, i in enumerate(range(2*num_quantized_bins - 1,
                                num_bins, 2*num_quantized_bins)):
        p_bin_idx_stop = i + 1
        thresholds[indexi] = hist_edges[p_bin_idx_stop]
        sliced_nd_hist = hist[:p_bin_idx_stop]

        # generate reference distribution p
        p = sliced_nd_hist.copy()
        assert p.size >= num_quantized_bins
        assert p_bin_idx_stop % num_quantized_bins == 0 and (p_bin_idx_stop // num_quantized_bins) % 2 == 0
        ...
        quantized_bins[0] = sliced_nd_hist[:num_merged_bins//2].sum()
        start = num_merged_bins//2
        # merge hist into num_quantized_bins bins
        for j in range(1, num_quantized_bins):
            stop = start + num_merged_bins
            quantized_bins[j] = sliced_nd_hist[start:stop].sum()
            start += num_merged_bins
        quantized_bins[-1] += sliced_nd_hist[start:].sum()

        # expand quantized_bins into p.size bins
        q = np.zeros(p.size, dtype=np.float32)
        start = 0
        stop = num_merged_bins//2
        norm = is_nonzeros[start:stop].sum()
        if norm != 0:
            q[start:stop] = float(quantized_bins[0]) / float(norm)
        start = num_merged_bins//2
        for j in range(1, num_quantized_bins):
            if j == num_quantized_bins - 1:
                stop = -1
        ...
        try:
            ...
            divergence[indexi] = float("inf")
        else:
            divergence[indexi] = stats.entropy(p, q)
        quantized_bins[:] = 0

    min_divergence_idx = np.argmin(divergence)
    min_divergence = divergence[min_divergence_idx]
    opt_th = thresholds[min_divergence_idx]
    return max_val, opt_th

if __name__ == "__main__":
    aa = torch.randn((512,256,100,100)).flatten()
    find_clip_mmse(aa, 4)
    find_clip_entropy(aa, 4)
